...
class _SpeakerTalksPageState extends State<SpeakerTalksPage> {
  ...
  Future<void> _openTalkDialog({Talk? editing, required String speakerId}) async {
    ...
    if (editing != null && programState.sections.isNotEmpty) {
      selectedSection = programState.sections.where((s) => s.id == editing.sectionId).firstOrNull;
    }
        ...
        content: SingleChildScrollView(
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                StatefulBuilder(
                  builder: (context, setDialogState) => DropdownButtonFormField<Section>(
                    value: selectedSection,
                    hint: const Text('Выберите секцию'),
                    items: programState.sections.map((s) => DropdownMenuItem(
                      value: s,
                      child: Text(s.name),
                    )).toList(),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              final newTalk = Talk(
                id: editing?.id ?? '',
                sectionId: selectedSection?.id ?? editing?.sectionId ?? '',
                speakerId: speakerId,
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 8),
                                        Text(
                                          'Секция: ${programState.sections.where((s) => s.id == talk.sectionId).firstOrNull?.name ?? talk.sectionId}',
                                          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                                                color: Theme.of(context)
                                                    .colorScheme
                                                    .onSurface
